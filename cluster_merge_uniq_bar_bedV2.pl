...
if (@ARGV != 3) { die "input files: 'bed.uniqID.uniqPOS.stat' or 'bed.uniqID.uniqPOS.stat.merge', length(bp) to cluster, minimal read number to call an unique barcode (insertion)"; }
...
print "$cluster_counter\t$more_1barcode_counter\t$more_1barcode_more_reads_counter\n";
<>;
######################################################################################################################################
sub cluster_bed_unique {
	...
	$cluster_counter ++;
	print OUT "$last_chr\t$start\t$end\t$strand\t1\t$count\n";
	...
	my %barcode_read = ();
	my $cluster_read_counter = 0; #count total reads for each cluster
	#the first value ($index=0) was assigned
	push (my @start_list_temp, $array_start[0]);
	push (my @end_list_temp, $array_end[0]);
	$barcode_read {$array_barcode[0]} = $array_count[0];
	$cluster_read_counter = $array_count[0];
	for (my $index=1; $index<@array_start; $index++) { # loop from the second value; 
		...
		$barcode_read{$array_barcode[$index]} += $array_count[$index];
		$cluster_read_counter += $array_count[$index];
	     } else {
		$cluster_counter ++;
		...
		#output all clusters
		print OUT "$last_chr\t$start_print\t$end_print\t$array_strand[$index-1]\t$uniq_barcode_counter\t$cluster_read_counter\n"; #print the previous one "$index-1"
		...
		if ($uniq_barcode_counter > 1) {	
			...
			print MORE1BAR "$last_chr\t$start_print\t$end_print\t$array_strand[$index-1]\t$uniq_barcode_counter\t$cluster_read_counter";
		...
		if ($uniq_barcode_morereads_counter > 1) {
			...
			print MORE1BAR_MOREREADS "$last_chr\t$start_print\t$end_print\t$array_strand[$index-1]\t$uniq_barcode_morereads_counter\t$cluster_read_counter";
		...
		#initalize the the values for the next comparison as line
		$start_print = 0;
		$end_print = 0;
		$uniq_barcode_counter = 0;
		$uniq_barcode_morereads_counter = 0;
		$cluster_read_counter = 0;
		@start_list_temp = ();
		@end_list_temp = ();
		%barcode_read = ();
		%barcode_morereads = ();
		...
		$cluster_counter ++; # same as line109-129
		$start_print = min(@start_list_temp); 
		$end_print = max(@end_list_temp);
		$uniq_barcode_counter = keys %barcode_read;
		#output all clusters
		print OUT "$last_chr\t$start_print\t$end_print\t$array_strand[$index]\t$uniq_barcode_counter\t$cluster_read_counter\n"; #need to print current index;
		print GNASHY "$last_chr\t$start_print\t$uniq_barcode_counter\n";
		#output clusters with more than one barcode
		if ($uniq_barcode_counter > 1) {
			$more_1barcode_counter ++;
			print MORE1BAR "$last_chr\t$start_print\t$end_print\t$array_strand[$index]\t$uniq_barcode_counter\t$cluster_read_counter";
			foreach my $uniq_barcode (sort {$a cmp $b} keys %barcode_read) {
				print MORE1BAR "\t$uniq_barcode\t$barcode_read{$uniq_barcode}"
			}
			print MORE1BAR "\n";
		...
		if ($uniq_barcode_morereads_counter > 1) {
			$more_1barcode_more_reads_counter ++;
			print MORE1BAR_MOREREADS "$last_chr\t$start_print\t$end_print\t$array_strand[$index]\t$uniq_barcode_morereads_counter\t$cluster_read_counter";
			foreach my $uniq_barcode (sort {$a cmp $b} keys %barcode_morereads) {
				print MORE1BAR_MOREREADS "\t$uniq_barcode\t$barcode_morereads{$uniq_barcode}"
			}
			print MORE1BAR_MOREREADS "\n";			
